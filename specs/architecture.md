...
検証スコア = (証拠検証数 × 0.5) + (論拠検証数 × 0.5) + 複数ユーザー検証ボーナス
  * 証拠検証数 = 証拠に対する検証投稿の数
  * 論拠検証数 = 論拠に対する検証投稿の数
...
いつくしみスコア = (対話促進度 + 包摂性 + 公平性 + 利他性) / 4
  * 対話促進度: 建設的な対話を促す度合い（0〜1）
    - 他者の視点を尊重し、対話を奨励する内容ほど高スコア
    - 敵対的・分断的な表現を避けた提案ほど高スコア
  * 包摂性: 多様な立場や視点を包含する度合い（0〜1）
    - 様々な背景や状況の人々に配慮した内容ほど高スコア
    - マイノリティ視点を考慮した提案ほど高スコア
  * 公平性: 公正さと偏りのなさの度合い（0〜1）
    - 客観的でバランスの取れた説明ほど高スコア
    - 特定の側に偏らない中立的記述ほど高スコア
  * 利他性: 共同体全体の利益を考慮する度合い（0〜1）
    - 個人的利益より集団の福祉を考慮した提案ほど高スコア
    - 長期的・持続的な解決策ほど高スコア
  * 注：いつくしみスコアは倫理委員会によるレビューと半自動評価を組み合わせて算出
