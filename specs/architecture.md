...
1. **時間経過による減衰**
   * 非アクティブ期間に応じた減衰: 分野内での活動がない場合、専門性スコアが時間とともに自動減少
     - 6か月の非アクティブ: 5ポイント減少
     - 1年の非アクティブ: 15ポイント減少
     - 2年の非アクティブ: 30ポイント減少
   * 最低保証スコア: レベル3以上のユーザーは、非アクティブでもレベル2未満には下がらない
   * リアクティベーション: 活動再開時に加速度的にスコア回復が可能

2. **証拠の無効化対応**
   * 重大な無効化: ユーザーが提供・検証した証拠が後に無効化された場合のペナルティ
     - 外部要因による無効化（科学的知見の更新など）: 軽微な減少（3ポイント）
     - 検証ミスによる無効化: 中程度の減少（5〜10ポイント）
     - 意図的な誤情報と判明: 大幅な減少（20〜30ポイント）と一時的な検証権限停止
   * 累積影響: 短期間に複数の証拠無効化が発生した場合、より大きなペナルティ
   * 異議申し立て: 無効化に対する異議申し立てプロセスあり

3. **検証精度の継続的評価**
   * 精度閾値: 直近30件の検証で精度が70%を下回った場合、警告とスコア減少
   * トレンド評価: 連続3回の精度低下が観測された場合、自動レベルダウン評価
   * 分野拡張ペナルティ: 専門外の分野での誤った検証は、より大きなペナルティ

4. **コミュニティフィードバック**
   * 信頼性フラグ: 複数の高レベルユーザーによる信頼性への疑義が提起された場合、レビュー実施
   * 多数決メカニズム: 同レベル以上のユーザー5人以上による評価減少提案でレビュー発動
   * 倫理違反: いつくしみスコアに関わる倫理違反行為が確認された場合の特別減点
